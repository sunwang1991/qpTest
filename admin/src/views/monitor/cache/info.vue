            ...
            <table cellspacing="0" style="width: 100%">
              <tbody>
                ...
                <tr>
                  ...
                  <td class="el-table__cell is-leaf">
                    <div class="cell" v-if="cache.info">
                      {{
                        parseFloat(
                          cache.info.cpu.used_cpu_user_children
                        ).toFixed(2)
                      }}
                    </div>
                  </td>
                ...
                <tr>
                  ...
                  <td class="el-table__cell is-leaf">
                    <div class="cell" v-if="cache.info">
                      {{ cache.info.stats.instantaneous_input_kbps }}
                      kps/
                      {{ cache.info.stats.instantaneous_output_kbps }}
                      kps
                    </div>
                  </td>
                </tr>
...
function getList() {
  ...
  getCache().then(response => {
    ...
    const memoryInfo = response.data.info.memory;
    const usedmemoryInstance = echarts.init(usedmemory.value, 'macarons');
    usedmemoryInstance.setOption({
      tooltip: {
        formatter: '{b} <br/>{a} : ' + memoryInfo.used_memory_human,
      },
      series: [
        {
          name: '峰值',
          ...
          max: 1000,
          detail: {
            formatter: memoryInfo.used_memory_human,
          },
          data: [
            {
              value: parseFloat(memoryInfo.used_memory_human),
              name: '内存消耗',
            },
          ],
        },
