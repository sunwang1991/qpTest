<template>
  <div class="app-container">
    ...
    >
      ...
      <el-form-item label="角色状态" prop="statusFlag">
        <el-select
    ...
    >
      ...
      <el-table-column label="角色状态" align="left" width="100">
        <template #default="scope">
          <el-switch
            v-if="
              scope.row.roleId !== SYS_ROLE_SYSTEM_ID &&
              !userStore.roles.includes(scope.row.roleKey)
            "
      ...
      >
        <template #default="scope">
          ...
          <el-tooltip
            content="数据权限"
            placement="top"
            v-if="scope.row.roleId !== SYS_ROLE_SYSTEM_ID"
          >
            <el-button
              link
              type="primary"
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="roleRef" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item label="菜单权限">
          ...
          <el-checkbox
            :checked="form.menuCheckStrictly === '1'"
    ...
    >
      <el-form :model="form" label-width="80px">
        ...
        <el-form-item label="数据权限" v-show="form.dataScope == 2">
          ...
          <el-checkbox
            :checked="form.deptCheckStrictly === '1'"
          ...
          <el-tree
            ...
            ref="deptRef"
            node-key="id"
            :check-strictly="form.deptCheckStrictly === '0'"
            empty-text="加载中，请稍候"
            :props="{ label: 'label', children: 'children' }"
          ></el-tree>
        </el-form-item>
...
function handleStatusChange(row) {
  ...
  proxy.$modal
    ...
    .then(function () {
      return changeRoleStatus(row.roleId, row.statusFlag);
    })
    .then(() => {
  ...
  getRole(roleId).then(response => {
    ...
    nextTick(() => {
      roleMenu.then(res => {
        ...
        menuNodeAll.value = checkedKeys.length > 0;
      });
    });
    title.value = '修改角色';
  });
...
function handleCheckedTreeConnect(value, type) {
  if (type == 'menu') {
    form.value.menuCheckStrictly = value ? '1' : '0';
  } else if (type == 'dept') {
    form.value.deptCheckStrictly = value ? '1' : '0';
  }
}
/** 所有菜单节点数据 */
  ...
  getRole(row.roleId).then(response => {
    ...
    nextTick(() => {
      deptTreeSelect.then(res => {
        ...
        deptNodeAll.value = checkedKeys.length > 0;
      });
    });
